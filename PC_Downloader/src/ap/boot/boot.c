...
uint8_t bootCmdReadVersion(uint8_t *p_version)
{
  bool ret;
  uint8_t errcode = OK;
  ...
  ret = cmdSendCmdRxResp(p_cmd, BOOT_CMD_READ_VERSION, NULL, 0, 500);
  if (ret == true)
  {
...
uint8_t bootCmdReadBoardName(uint8_t *p_str)
{
  bool ret;
  uint8_t errcode = OK;
  cmd_t *p_cmd = &cmd_boot;
  ...
  ret = cmdSendCmdRxResp(p_cmd, BOOT_CMD_READ_BOARD_NAME, NULL, 0, 1000);
  if (ret == true)
  {
    for (i=0; i<p_cmd->rx_packet.length; i++)
    {
